    ...
    this.build = function(commitObj){
        ...
        conf.forEach(function(build){
            ...
            exec(build.process, {
                ...
                if (err !== null) {
                    cli.out(0, ("Build failed ("+build.key+") ").red);
                    cli.out(0, "stdout:".cyan);
                    cli.out(0, stdout.white);
                    cli.out(0, "stderr:".cyan);
                    cli.out(0, stderr.red);

                    deferredBuild.reject(stdout+'\n\n'+stderr);
                }else{
...
var getProject = function(){
    ...
    try {
        ...
        cli.error(new Error("Could not find local "+projectConfFile+" file. \n\nCheck you are executing stash-build from the correct directory, or run `stash-build init` to create a new configuration"), 1);
        return false;
    }
